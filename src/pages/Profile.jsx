...
const getOrderStatusColor = (status) => {
  ...
  if (status === 'failed' || status === 'expired' || status === 'cancelled') return '#dc2626';
  return '#6b7280';
};
...
const IKey = (p) => <SvgWrap {...p}><circle cx="7.5" cy="15.5" r="5.5"/><path d="M21 2l-9.6 9.6"/><path d="M15.5 7.5l3 3L22 7l-3-3"/></SvgWrap>;
...
const ProfilePage = () => {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [pR, aR, oR] = await Promise.all([
        fetch(`${API_BASE}/profile`, { headers: h }),
        fetch(`${API_BASE}/access/my`, { headers: h }),
        fetch(`${API_BASE}/purchase/history?limit=5`, { headers: h }),
      ]);
      const [pD, aD, oD] = await Promise.all([pR.json(), aR.json(), oR.json()]);
      ...
      if (pD.status) {
        ...
        showToast(pD.message || 'Gagal memuat profil', 'error');
      }
      if (aD.status) setAccessList(aD.data || []);
      if (oD.status) setOrders(oD.data || []);
    } catch {
  ...
  // Membership aktif diambil dari access/my (access_type === 'membership', masih aktif & belum expired)
  const activeMembership = accessList.find((a) => {
    if (a.access_type !== 'membership' || !a.is_active) return false;
    if (a.expires_at && new Date(a.expires_at) < new Date()) return false;
    return true;
  });
  const isPremium   = Boolean(profile.has_active_membership && activeMembership);
  const memberColor = '#DC1F2E';
  const memberLabel = activeMembership?.label || 'Membership';
  const remainingDays = activeMembership ? daysRemaining(activeMembership.expires_at) : null;

  const showAccessList = accessList.filter((a) => a.access_type === 'pershow' && a.is_active
        ...
        <div className="pp-hero">
          <div className="pp-hero__top">
            ...
            <div className="pp-hero__info">
              ...
              <div className="pp-hero__badges">
                ...
                <span className="pp-badge"
                  style={{
                    color: profile.is_active ? '#4ade80' : '#ff4757',
                    background: profile.is_active ? 'rgba(22,163,74,0.15)' : 'rgba(220,31,46,0.15)',
                    border: `1px solid ${profile.is_active ? 'rgba(22,163,74,0.3)' : 'rgba(220,31,46,0.3)'}`,
                  }}>
                  <IShield size={11} /> {profile.is_active ? 'Aktif' : 'Nonaktif'}
                </span>
              </div>
            </div>
          ...
          <div className="pp-hero__stats">
            <div className="pp-hero__stat">
              <span className="pp-hero__stat-num pp-info-value--mono">{profile.user_code || '—'}</span>
              <span className="pp-hero__stat-label">Kode User</span>
            </div>
            <div className="pp-hero__stat-divider" />
            <div className="pp-hero__stat">
              <span className="pp-hero__stat-num" style={{ color: isPremium ? memberColor : '#9ca3af' }}>
                {isPremium ? `${remainingDays} hari` : '—'}
              </span>
              <span className="pp-hero__stat-label">Sisa Membership</span>
            </div>
            <div className="pp-hero__stat-divider" />
            <div className="pp-hero__stat">
              <span className="pp-hero__stat-num" style={{ color: '#9ca3af' }}>
                {profile.active_show_access_count ?? showAccessList.length}
              </span>
              <span className="pp-hero__stat-label">Akses Show Aktif</span>
            </div>
          </div>
        </div>
          ...
          <div className="pp-grid">
            <div className="pp-col">
              <div className="pp-card">
                ...
                <div className="pp-info-list">
                  ...
                  <InfoRow icon={IKey}      label="Kode User"     value={profile.user_code || '—'} mono />
            ...
            <div className="pp-col pp-col--sidebar">
                ...
                {isPremium ? (
                  <>
                    ...
                    <div className="pp-info-list" style={{ marginTop: 12 }}>
                      <InfoRow icon={ICalendar} label="Mulai"    value={formatDate(activeMembership.created_at)} />
                      <InfoRow icon={ICalendar} label="Berakhir" value={formatDate(activeMembership.expires_at)} />
                      <InfoRow icon={IClock}    label="Sisa"
                        value={`${remainingDays} hari`} valueColor={memberColor} />
                    </div>
                  </>
                ) : (
                  <div className="pp-app-notice">
                    <IStar size={28} color="#7b1c1c" />
                    <p className="pp-app-notice__text">
                      Kamu belum memiliki membership aktif. Beli paket membership atau akses pershow lewat menu order.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="pp-card">
                ...
                <p className="pp-user-id">{profile.id}</p>
              </div>
            </div>
          </div>
        )}
            ...
            {isPremium ? (
              <>
                  ...
                  <div className="pp-membership-progress-wrap">
                    <div className="pp-membership-progress" style={{ background: `${memberColor}22` }}>
                      <div className="pp-membership-progress__fill"
                        style={{ background: memberColor, width: `${Math.min(100, (remainingDays / 30) * 100)}%` }} />
                    </div>
                  </div>
                </div>
                <div className="pp-info-list" style={{ marginTop: 16 }}>
                  <InfoRow icon={ICalendar} label="Mulai"    value={formatDate(activeMembership.created_at)} />
                  <InfoRow icon={ICalendar} label="Berakhir" value={formatDate(activeMembership.expires_at)} />
                  <InfoRow icon={IStar}     label="Paket"    value={memberLabel} valueColor={memberColor} />
              ...
              <div className="pp-app-notice pp-app-notice--lg">
                <div className="pp-app-notice__icon-wrap">
                  <IStar size={36} color="#7b1c1c" />
                </div>
                <h3 className="pp-app-notice__title">Belum memiliki membership</h3>
                <p className="pp-app-notice__text">
                  Beli paket membership untuk akses semua livestream, atau beli akses pershow untuk
                  livestream tertentu.
                </p>
              </div>
            )}

            {showAccessList.length > 0 && (
              <div style={{ marginTop: 20 }}>
                <h3 className="pp-card__subtitle">
                  <IBag size={14} color="#7b1c1c" /> Akses Pershow Aktif
                </h3>
                <div className="pp-order-list">
                  {showAccessList.map((a) => (
                    <div key={a.id} className="pp-order-item">
                      <div className="pp-order-item__left">
                        <p className="pp-order-item__plan">{a.show_title || a.label}</p>
                        <p className="pp-order-item__id">Kode: {a.access_code}</p>
                      </div>
                      <div className="pp-order-item__right">
                        <p className="pp-order-item__amount">
                          {a.expires_at ? `Sampai ${formatDate(a.expires_at)}` : 'Tanpa batas waktu'}
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              <div className="pp-order-list">
                {orders.map((o) => (
                  <div key={o.order_id} className="pp-order-item">
                    <div className="pp-order-item__left">
                      <p className="pp-order-item__plan">{o.plan_name || o.show_title || (o.order_type === 'membership' ? 'Membership' : 'Pershow')}</p>
